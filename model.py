...
class HMM:
    def __init__(self, items: List[str], h_states: Optional[int] = 4, skip_init: bool = False):
        if skip_init:
            return
        N = h_states
        M = len(items)
        P = [1/N] * N
        trans_mat = self._rand_norm_mat(N, N)
        dists = list()
        for i in range(N):
            probs = self._rand_norm_mat(1, M)[0]
            b_qi = dict()
    ...
    def train(self, data: List[int], indices: List[int], options: Dict[str, str] = None) -> float:
        if not options:
            options = self.options
        self.indices = indices
        imp = self.model.fit(
            (data.sequence(i) for i in indices), 
            verbose=True, 
            n_jobs=4,
    ...
    def _rand_norm_mat(self, n_rows: int, n_cols: int, seed: int = 0):
